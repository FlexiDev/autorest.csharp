    ...
    public class Program : NewPlugin
    {
        ...
        protected override async Task<bool> ProcessInternal()
        {
            ...
            else
            {
                ...
                // build settings
                var inputFile = (await GetValue<string[]>("input-file") ?? new[] { "" }).FirstOrDefault();
                var altNamespace = inputFile?.Split('/').Last().Split('\\').Last().Split('.').First();
                ...
                Settings.Instance.CodeGenerationMode = (await GetValue<bool?>("rest-server") ?? false) ? "rest-server" : "rest-client";
